...
The LTS supports testing with standard JavaScript test frameworks. We provide easy-to-use wrappers for using [Jasmine](https://jasmine.github.io/) and [Mocha](http://mochajs.org/). If you’d like to use an alternative test framework, you’ll have to wrap it yourself. (See [Next Steps](#next-steps) for some details.)
 ...
 * If you plan to evaluate or use LTS in more depth, you’ll want to use it with [Salesforce DX](https://developer.salesforce.com/platform/dx). Once you install the Salesforce DX CLI, you can work with your test suite in a variety of ways from the command line. This approach is recommended for systematic automated testing.
...
There are two stages of installing the LTS. First install the unmanaged package. Then, optionally, install the latest Salesforce CLI plugin to use the LTS with the `sfdx` command line tool.
...
## Installing the Lightning Testing Service for Salesforce DX

The Salesforce DX CLI allows you to use the `sfdx` command line tool to perform automated testing as part of your development process, including automated process, such as continuous integration.

### Install the Salesforce DX CLI

1. Install the Salesforce DX CLI by following the instructions for your operating system.
...
2. Verify that the Salesforce DX CLI plug-in is installed and updated by running the following command in your shell or terminal application:
   ...
   Additional verification details are available in the relevant Salesforce DX documentation: https://developer.salesforce.com/docs/atlas.en-us.sfdx_setup.meta/sfdx_setup/sfdx_setup_install_cli_verify.htm

After you install the Salesforce DX CLI, you can run your tests from the command line using the `sfdx` tool. For example:
 ...
 * `jasmineExampleTests.resource` — The Jasmine example test suite that’s worth your time to review.
 ...
 * `mochaExampleTests.resource` — The Mocha example test suite, which provides examples parallel to the corresponding Jasmine test suite.
...
The `jasmineExampleTests.resource` and `mochaExampleTests.resource` files are each a single JavaScript file containing a complete test suite. It’s a single file for convenience in delivery and exploration. Your own test suites can include many such files. Both test suites are copiously commented. The code and comments serve as the official documentation for how to write tests.
 ...
 * `lts_jasmineboot.resource` — A JavaScript IIFE that launches Jasmine in the LTS context, and adapts results for the HTML report.
  * `lts_testutil.resource` — A collection of utilities for use within your test specs. They provide Lightning component-specific functions that make it easier to test your custom components from a test context.
